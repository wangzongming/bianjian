...
// import { Route, Redirect } from "react-router"; //,Router HashRouter as Router,
            ...
            for (let j = 0; j < routeData.length; j++) {
                ...
                if (componentKey === routeItem["componentKey"]) {
                    ...
                    if (index && !_haveIndex && componentKey) {
                        ...
                        _routes.push(<Redirect key={j} exact from="/" to={route} />);
                        _haveIndex = true;
                    ...
                    if (author) {
                        ...
                        if (store.get("token")) {
                            ...
                            //没有token就跳转到登陆页面 
                            // _routes.push(<Redirect key={j} exact from="/" to={route} />); 
                            _routes.push(<Redirect key={j} to={"/login"} />);
                        }
                    } else {
                        _routes.push(<Route key={j} component={routeItem["component"]} path={route} />);
                    }
                }
            }
