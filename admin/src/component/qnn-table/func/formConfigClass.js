...
const formConfigClass = function () {
    ...
    for (let i = 0; i < formConfig.length; i++) {
        ...
        if (isInTable) {
            ...
            if (type === "images") {
                ...
                table.render = data => {
                    if (data) {
                        return this.numberFormat(data,decimals);
                    } else {
                        return defaultValue;
                    }
                };
            ...
            if (onClick) {
                table.render = (data,rows,index) => {
                    ...
                    //进行最后的 是否可以点击判断
                    //isCanClick可为func  和  object
                    if (isCanClick) {
                        let _isCanClick = "";
                        if ((typeof isCanClick) === 'function') {
                            ...
                            _isCanClick = true;
                            for (const key in isCanClick) {
                                if (!rows[key] || rows[key] !== isCanClick[key]) {
                                    _isCanClick = false;
                                }
                            }
                        }
    ...
    this.setState({
        ...
        forms,
        searchFroms,
        tableContentWidth: colNumber > 7 ? _tableContentWidth : "100%",
