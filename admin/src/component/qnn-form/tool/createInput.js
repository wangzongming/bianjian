...
import { General,Linkage,QnnFormBlock } from "../components";
...
const createInput = function (_params,index) {
    ...
    if (fetchConfig && ((typeof fetchConfig) === "function")) {
        params.fetchConfig = fetchConfig({
            ...this.funcCallBackParams
        });
    }
    ...
    //特殊类型的初始值处理
    if (
        (type === "date" || type === "time" || type === "datetime" || type === "month" || type === "year") &&
        this.isMobile() === false
    ) {
    ...
    const _props = {
        ...
        setState: (obj,cb) => this.setState({ ...obj },cb),
        ...
        qnnFormIsMounted: this.isMounted
    };
            ...
            return (
                <div>
                    {form.getFieldDecorator(field,{
                        ...
                        //开发中...
                        // setFormBlockObj={(field,me) => {
                        //     if (!this.formBlocks) { this.formBlocks = {} };
                        //     this.formBlocks[field] = me;
                        //     // console.log(this.formBlocks)
                        // }}
                        />
                    )}
                </div>
