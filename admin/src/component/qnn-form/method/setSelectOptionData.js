...
const setSelectOptionData = function (cb) {
    ...
    const { form: { getFieldValue = () => null } } = this.props;
    ...
    const setFormOption = formConfig => {
        for (let i = 0; i < formConfig.length; i++) {
            ...
            if (type === "linkage") {
                ...
                //设置普通的下拉选项
                const { apiName,otherParams = {},params = {} } = fetchConfig;
                if ((type === "select" || type === "cascader") && apiName) { // || type === "selectByPaging"
                    const selectKey = this.selectKey(field);
                    ...
                    const urlParams = this.props.match.params;
                    for (const key in params) {
                        _params[key] = urlParams[params[key]] || getFieldValue(params[key]);
                    }
                    this.setState({ loading: true });
                    this.myFetch(
                        apiName,
