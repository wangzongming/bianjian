    ...
    if (type === "set") {
        for (let i = 0; i < formConfig.length; i++) {
            let {
                type,
                field,
                oldfield, //表单块的field会被更改 这是更改前备份的  这个存在的话就不应该去取 field || dataIndex;
                dataIndex,
                defaultValue,
                initialValue,
                multiple,
            ...
            field = oldfield || field || dataIndex;
            //不在设置的数据中的字段直接跳过
            ...
            if (typeof initialValue === "function") {
                if (this) {
                    ...
                    initialValue = initialValue({ ...props });
                }
            }
...
const setValues = function (data) {
    ...
    const { isQnnFormBlock } = this.props;
    ...
    if (isQnnFormBlock) {
        //表单块需要在设置的值前面吧父级字段加上不然设置不上去 
        let _datas = {};
        for (let i = 0; i < _formConfig.length; i++) {
            let item = _formConfig[i];
            let { field,oldfield } = item;
            _datas[field] = newData[oldfield];
        }
        setFieldsValue(_datas);
    } else {
