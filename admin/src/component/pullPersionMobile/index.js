...
class PullPersionMobile extends Component {
    ...
    k = {
        ...
        type: this.props.k
            ? this.props.k.type
                ...
                : "type"
            : "type",
        children: this.props.k
            ? this.props.k.children
                ? this.props.k.children
                : "children"
            : "children"
    };
    ...
    textObj = {
        ...
        //底部选中的列表的标题
        subTit: this.props.textObj
            ? this.props.textObj.subTit
            : "已选择的部门或成员",
        //添加按钮的文字
        addBtn: this.props.textObj ? this.props.textObj.addBtn : "添加",
        //当selectType===1时弹出的提示
        noSelectPerson: this.props.textObj
            ? this.props.textObj.noSelectPerson
            : "只能选择部门！",
        //当selectType===2时弹出的提示
        noSelectDepartment: this.props.textObj
            ? this.props.textObj.noSelectDepartment
            : "只能选择人员！",
        //根节点不让选时弹出的提示
        rootNodeNoSelect: this.props.textObj
            ? this.props.textObj.rootNodeNoSelect
            : "根节点不可被选择！",
        //右上角保存按钮文字
        saveBtn: this.props.textObj ? this.props.textObj.saveBtn : "保存",
        cancelBtn: this.props.textObj ? (
            this.props.textObj.cancelBtn
        ) : (
            <Flex>
                <Icon type="left" />
                <span>取消</span>
        ...
        //最大值最小值出限提醒
        maxNumber: this.props.textObj
            ? this.props.textObj.maxNumber
            : "选择数量达到了上限",
        minNumber: this.props.textObj
            ? this.props.textObj.minNumber
            : `选择数量不足，至少需要选择${this.state.minNumber}个`
    };
    ...
    componentWillReceiveProps(newProps) {
        ...
        this.defaultValue = newProps.defaultValue
    ...
    componentDidMount() {
                    ...
                    if (routeInfo.pathname === this.parentUrl) {
                        if (navData.length > 1) {
                            ...
                            this.setState({
                                ...
                                listData:
                                    navData.length > 1
                                        ? navData[navData.length - 1][children]
                                        : this.treeData
                            });
        ...
        if (fetchConfig.apiName) {
            ...
            this.myFetch(fetchConfig.apiName, fetchConfig.params).then(
    ...
    CheckboxItemDoubleClick(e, data) {
        ...
        if (_type !== "2" && _children) {
            ...
            this.setState({
                ...
                //设置底部显示的信息
                bottomInfoList:
                    _value === bottomInfoId ? bottomInfoList : _bottomInfoList
            });
        }
    }
    ...
    render() {
        ...
        const formCon = this.defaultValue
            ? this.defaultValue.map((item, index) => {
                  ...
                  return (
                      <div
                          key={index}
                          style={{ paddingRight: edit ? "25px" : "5px" }}
                          className={`${s.fItem}  ${
                              _type === "2" ? s.per : s.folder
                          }`}
                      >
                          {_label}
                          <span
        ...
        //渲染导航条
        const renderNav = (
            <div className={"pull-person-mobile-navContainer"}>
        ...
        const renderList = (
            <div>
                {
                    <List
                        className={"pull-person-mobile-myList"}
                        ...
                        >
                            {listData && listData.length ? (
                                listData.map((item, index) => {
                                    ...
                                    //根节点不可选的时候
                                    if (!this.rootNodeSelect && _type === "0") {
                                        //根节点
                                        _img = imgs.blueFolderImg;
                                        _isCheck = false;
                                    } else if (
                                        ...
                                        >
                                            {_isCheck ? (
                                                <CheckboxItem
                                                    ...
                                                    onChange={e =>
                                                        this.PonChange(e, valO)
                                                    }
                                                >
                                                    {_label}
                                                    {this.props.useCollect ? collectBtn : null}
                                                </CheckboxItem>
                                            ) : (
                                                <List.Item
                                                    onClick={e => {
                            ...
                            {/* 渲染收藏的联系人 */}
                            {navData.length === 1 && !searchValue && this.props.useCollect ? (
                                <div className={s.collectionContainer}>
                                    <div className={s.title}>收藏</div>
                                    <div className={s.collectionCon}>
                                        {!collect.length ? (
                                            <div
                                                style={{
                                                    color: "#ccc",
                            ...
                            {/* <div className={s.collectionContainer}>
                                <div className={s.content}>
                                    <div className={s.collectionContainerBtn}>
                                    ☆
                                    </div>
        ...
        //主内容
        const sidebar = (
            <div className={s.pullPersonContainer}>
                <div>
                    {noBar ? (
                        ""
                    ) : (
                        <NavBar
                            ...
                            icon={
                                <Flex>
                                    <div>{this.textObj.cancelBtn}</div>
                                </Flex>
                            }
                            onLeftClick={this.cancelFn}
                            rightContent={
                                <div onClick={this.saveFn}>
                                    {this.textObj.saveBtn}
                                </div>
                            }
                            {...this.NavBarAntd}
                        >
                ...
                {/* 节点渲染 */}
                <div className={"pull-person-mobile-treeContainer"}>
                    <WhiteSpace />
                    {loading ? (
                        <ActivityIndicator
                            className={"myActivityIndicator"}
                            text={this.textObj.loading}
                        />
                    ) : treeData ? (
                        renderList
                    ) : (
                        <div className={s.noData}>{this.textObj.noData}</div>
                    )}
                    <WhiteSpace />
                </div>
                {/* 底部信息框 */}
                <div
                    className={"pull-person-mobile-bottomInfo"}
                    style={{
                        transform: bottomInfoShow
                            ? "translateY(0)"
                            : "translateY(100%)",
                        WebkitTransform: bottomInfoShow
                            ? "translateY(0)"
                            : "translateY(100%)",
                        ...bottomInfoStyle
                    }}
                >
                    ...
                    <div className={"content"}>
                        {bottomInfoList.length === 0 ? (
                            <div style={{ color: "#ccc" }}>无数据</div>
                        ) : null}
        ...
        return (
            <div className={s.PullPersionMobile}>
                ...
                >
                    <div className={"pull-person-mobile-myDrawer"}>
                        {sidebar}
                    </div>
                </Modal>
            </div>
